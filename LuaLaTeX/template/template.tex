\documentclass[a4paper,10pt]{ltjsarticle} % LuaLaTeX用の標準クラス
\usepackage[margin=25mm]{geometry} % 余白設定（これ1行で上下左右の余白を美しく設定できるわ）
...
% ==========================================
% 【LaTeXを書く時の絶対ルール】
% 1. 「\」や「スペース」は必ず半角入力！全角スペースはエラーの最大の原因よ！
% 2. 「%」の後ろに書いた文字はコメントになる（PDFには表示されず、メモに使えるわ）
...
\begin{center} % タイトルの部分
  ...
  {\large \textbf{電気通信大学 情報理工学域 ◯類}} \\ % 自分の類に書き換えること
...
\begin{figure}[H] % Hをつけるとここに強制的に配置される。慣れないうちはこれを使うとよい
  \centering
...
% （初回）左上のメニューから、左のTeXタブから『レシピ: lualatex』を選んでビルド

% 環境によってまちまちなのでそこは自分で調べたり生成AIに聞いたりでなんとかしましょう